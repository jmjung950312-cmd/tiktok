...
const segs = [
  ...
  { index: 4, text: '연애에서 가장 흔한 실수 다섯 가지를 알려드릴게요', startMs: 6000, endMs: 12000, audioPath: '' },
];

console.log('=== default mode ===');
const def = buildAssSubtitle({ jobId: 'test', itemIndex: 0, segmentTimings: segs, tempDir, mode: 'default' });
console.log('mode:', def.mode);
console.log(readFileSync(def.subtitlePath, 'utf-8').split('[Events]')[1]);

console.log('');
console.log('=== karaoke mode ===');
const kar = buildAssSubtitle({ jobId: 'test', itemIndex: 1, segmentTimings: segs, tempDir, mode: 'karaoke' });
...
const env = buildAssSubtitle({ jobId: 'test', itemIndex: 2, segmentTimings: segs.slice(0, 1), tempDir });
